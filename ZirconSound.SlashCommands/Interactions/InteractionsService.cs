...
public class InteractionsService : IInteractionsService
{
    ...
    private static void SetMessageComponentParameters(ref object[] parameterArray, InteractionsData interactionData, MethodBase method)
    {
    ...
    private async Task<KeyValuePair<InteractionsData, IInteractionGroup>> GetDataFromSocket(SocketInteraction interaction)
    {
        IInteractionGroup commandToExec = new InteractionsGroup();
        var interactionData = new InteractionsData();
        switch (interaction)
        {
        ...
        return new KeyValuePair<InteractionsData, IInteractionGroup>(interactionData, commandToExec);
    }
    ...
    private async Task ExecuteInternalAsync(SocketInteraction interaction, IInteractionContext context)
    {
        try
        {
            ...
            {
                await _messageComponentExecutedEvent.InvokeAsync(interaction as SocketMessageComponent, context, new InteractionsResult("Success", true)).ConfigureAwait(false);
            }
        }
        catch (Exception ex)
        {
            ...
            {
                await _commandExecutedEvent.InvokeAsync(command, context, new InteractionsResult(CommandError.Exception, ex.Message, false)).ConfigureAwait(false);
            }
            else
            {
                await _messageComponentExecutedEvent.InvokeAsync(interaction as SocketMessageComponent, context, new InteractionsResult(CommandError.Exception, ex.Message, false)).ConfigureAwait(false);
            }
        }
    }
    ...
    public async Task Invoke(SocketInteraction component, IInteractionContext context)
    {
        ...
        switch (_config.DefaultRunMode)
        {
            case RunMode.Sync:
                ...
                break;
            case RunMode.Async:
                await TaskAsync.Run(async () => { await ExecuteInternalAsync(component, context).ConfigureAwait(false); });
                break;
